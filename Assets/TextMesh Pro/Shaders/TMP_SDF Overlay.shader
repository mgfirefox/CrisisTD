    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            pixel_t VertShader(vertex_t input)
            {
                ...
                pixelSize /= float2(_ScaleX, _ScaleY) * abs(
                    mul((float2x2)UNITY_MATRIX_P, _ScreenParams.xy));
                float scale = rsqrt(dot(pixelSize, pixelSize));
                scale *= abs(input.texcoord1.y) * _GradientScale * (_Sharpness + 1);
                if (UNITY_MATRIX_P[3][3] == 0)
                    scale = lerp(abs(scale) * (1 - _PerspectiveFilter),
                        scale, abs(dot(UnityObjectToWorldNormal(input.normal.xyz),
                ...
                float alphaClip = (1.0 - _OutlineWidth * _ScaleRatioA - _OutlineSoftness *
                    _ScaleRatioA);

                #if GLOW_ON
                alphaClip = min(alphaClip,
                             1.0 - _GlowOffset * _ScaleRatioB - _GlowOuter *
                             _ScaleRatioB);
                #endif
                ...
                float bBias = (0.5 - weight) * bScale - 0.5 - ((_UnderlayDilate * _ScaleRatioC) *
                    0.5 * bScale);
      ...
      _WorldSpaceCameraPos.xyz - mul(unity_ObjectToWorld, vert).xyz);
            ...
            fixed4 PixShader(pixel_t input) : SV_Target
            {
                ...
                float3 bump = UnpackNormal(tex2D(
                        _BumpMap,
                        input.textures.xy + float2(_FaceUVSpeedX, _FaceUVSpeedY) * _Time.y)).
                    xyz;
                ...
                faceColor += input.underlayColor * saturate(d - input.texcoord2.w) * (1 - faceColor.
                    a);
                #endif
